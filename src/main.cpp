...
#if defined(NDEBUG)
# error "elikeme cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x408c1a44c7c377cd2b90f4c310e7310588bc904f3fd4dcc5eb6631312d974007");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // elikeme: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "elikeme Signed Message:\n";
...
{
    // elikeme: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // elikeme: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 30; // elikeme: 3.5 days
static const int64 nTargetSpacing = 1 * 30; // elikeme: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // elikeme: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // elikeme: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x408c1a44c7c377cd2b90f4c310e7310588bc904f3fd4dcc5eb6631312d974007");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "NY Times 02/Jan/2018 ShangHai Elikme fuckall";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04a1e78954b5bdcda918c256eaa7a67178c53dc86c35e9b32abd3a7f626a0fbb1b8f994d29f98e5db22ddb2bdd0fdf34a324bfa618982ed98d564186fb14cc25bf") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1517467695;
        ...
        assert(block.hashMerkleRoot == uint256("0xe677edc78bfc7126b0200850186c259f0d7275ab6b7ba5393436b8518f08e41f"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // elikeme: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("elikemeMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("elikemeMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static elikemeMiner(CWallet *pwallet)
{
    printf("elikemeMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("elikeme-miner");
    ...
    try { loop {
        ...
        printf("Running elikemeMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("elikemeMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&elikemeMiner, pwallet));
}
